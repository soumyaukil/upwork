...
static uint32_t randnu(uint32_t i){
	uint32_t ret;
...
uint32_t encrypt::GetSysID(bool bShowID)
{
	uint32_t ret;	
	uint32_t diskserialnum;
	uint32_t osserialnum;
	uint32_t sysserialnum;
	char buff[100];
	uint32_t maxlen;
	uint32_t flag;
	char namebuff[100]; 
...
bool encrypt::EncryptLicenseSt(char *psrc, char *pdec,uint32_t sysid)
{
	int seed;
	int i;
	uint8_t tmp;
...
bool encrypt::DecryptLicenseSt(char *psrc, char *pdec, uint32_t sysid)
{
	int seed;
	int i;
	uint16_t sum1,sum2;
