...
public:
	...
	int GetCounter();
	inline int GetHashSize() { return hashsize;};
	stritem *FindItemVal(const char* key,long val);
	int RemoveAll();
	static uint32_t HashKey(const char* key);
	stritem* Enum(stritem *&pitem);
	stritem* EnumKey(const char* key,stritem *&pitem);
	void EnumFirst(stritem ** ppitem);
	void EnumNext(stritem **ppitem);
	stritem * EnumFirst();
	stritem * EnumNext(stritem *pitem);

	void EnumFirstKey(const char* key, stritem **ppitem);
	void EnumNextKey(const char* key,stritem **ppitem);

	stritem * FindItem ( const char* key );
	void UnlockMap();
	void LockMap();
	stritem *Add(const char* key, long val);
	stritem *AddNoDup(const char* key, long val);
	int Remove(const char* key);
	int RemoveAllDup(const char* key);
	int RemoveVal(const char* key, long val);
	stritem *AddAscent(const char* key,long val);
	int GetMaxBucket();
	void init(int size);
	strmap(int size);
	strmap();
