...
BOOL CControlPos::AddControl(CWnd* pControl, const DWORD& dwStyle /* = CP_MOVE_HORIZONTAL */)
{
	...
	if (pControl && m_pParent)
	{
		...
		pControl->GetWindowRect(pstControl->mrect);
		m_pParent->ScreenToClient(pstControl->mrect);
...
void CControlPos::SetParent(CWnd *pParent, int x, int y)
{
//	CRect rcParentOriginalSize;
...
//	m_pParent->GetClientRect(rcParentOriginalSize);
	m_nOriginalParentWidth = x;
	m_nOriginalParentHeight = y;
	UpdateParentSize();
