...
BOOL CStrFunc::IsTestSec(LPCSTR secsym)
{
...
DWORD CStrFunc::GetASCIINumber(LPCSTR numberbuff, int len)
{
	register DWORD ret;
	register char cc;
	ret=0;
...
int CStrFunc::GetCommaIntVal(LPCSTR pstr)
{
	int ret;
	int sign;
	char c;
...
int CStrFunc::GetTime_HMS(LPCSTR pstr)
{
	int h,m,s;
	h=m=s=0;
...
BOOL CStrFunc::FillFont(LOGFONT *pfont, LPCSTR lpstr)
{
	int i;
...
CString CStrFunc::FormatFloat(double dbl)
{
	char str[100];
	CString ret;
	sprintf(str,"%lf",dbl);
...
BOOL CStrFunc::TransOptSymb(LPCSTR sfrom, char *sto, BOOL btoplus)
{
	int i,j;
	if (btoplus) {
		i=0;j=0;
...
BOOL CStrFunc::IsOptionSymb(LPCSTR secsym)
{
...
char * CStrFunc::ExtractString(LPCSTR psrc, int len,char *pdesc)
{
...
char * CStrFunc::FormatIPAddr(DWORD ip, char *pbuff)
{
	if (pbuff==NULL) {
		return "";
	}else {
...
char* CStrFunc::InsertSpace(LPCSTR psrc, char *pdest)
{
	int i;
	int j=0;
	for (i=0;psrc[i];i++){
