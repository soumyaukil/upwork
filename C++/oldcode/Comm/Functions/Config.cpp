...
static uint32_t GetFileVersion(const char* filename) 
{
	uint32_t ret=0;
	...
	uint8_t *pData = new uint8_t [dwSize + 1];	
	ZeroMemory(pData, dwSize+1);
	
	rc = ::GetFileVersionInfo(filename, dwHandle, dwSize, pData);
	if (!rc) {
	} else {

		UINT nLength;
		VS_FIXEDFILEINFO *pFixedInfo = NULL;
		rc = ::VerQueryValue(pData, _T("\\"), (void **) &pFixedInfo, &nLength);
		if (rc){
			ret= HIWORD(pFixedInfo->dwProductVersionMS)<<24;
			ret+=LOWORD(pFixedInfo->dwProductVersionMS) <<16;
			ret+=HIWORD(pFixedInfo->dwProductVersionLS) <<8;
			ret+=LOWORD(pFixedInfo->dwProductVersionLS);
...
int CConfig::ShowConfigFile(const char* filename)
{
	char profilename[400];
	if (filename==NULL) {
		sprintf(profilename,"%s%s",mAppPath,mAppCfgFile); 
	...
	ShellExecute(NULL,"open","notepad",profilename,NULL,SW_SHOWNORMAL ); 
	return 0;
}
...
bool CConfig::GetIpPortVal(const char* str, uint32_t *ip, WORD *port)
{
	uint32_t mip;
	WORD mport;
	char st[100];
...
bool CConfig::FormatTimeYMDHMS(time_t ntime, char *pbuff)
{
	...
	if (ptmTemp == NULL ||
		!_tcsftime(pbuff, 20, "%m/%d/%y %H:%M:%S", ptmTemp))
		pbuff[0] = '\0';
	return true;
}
...
bool CConfig::FormatTimeHMS(time_t ntime, char *pbuff)
{
	...
	struct tm* ptmTemp = localtime(&ntime);
	if (ptmTemp == NULL ||
		!_tcsftime(pbuff, 10, "%H:%M:%S", ptmTemp))
		pbuff[0] = '\0';
	return true;
}
