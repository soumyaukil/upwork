...
{
   return RecordExceptionInfo(pExInfo, "DAS Error handler");
//   return EXCEPTION_EXECUTE_HANDLER;
}



static LPTOP_LEVEL_EXCEPTION_FILTER  m_oldFilter;      // previous exception filter

void CrashReportInstall()
{
	if (m_oldFilter==NULL) m_oldFilter = SetUnhandledExceptionFilter(CustomUnhandledExceptionFilter);

}

void CrashReportUninstall()
{
	if (m_oldFilter)      SetUnhandledExceptionFilter(m_oldFilter);
}
