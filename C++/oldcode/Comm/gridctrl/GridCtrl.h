...
protected:
	CScrollBar *m_VScrollBar; //add by tom
	CScrollBar *m_HScrollBar; //add by tom 
	int m_VScrollPos;
	int m_HScrollPos;
...
public:
	...
	BOOL CreateFromStatic(UINT nID, CWnd* parent);
...
public:
    ...
    BOOL SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data,
                   int low, int high);
	BOOL SortNumItems(int nCol, BOOL bAscending);
	BOOL SortNumItems(int nCol, BOOL bAscending, int low, int high);
...
public:
	...
	int SetScrollBar(CScrollBar *phscrollbar, CScrollBar *pvscrollbar);
	BOOL SetItemTextLong(int nRow, int nCol, long val);
	long GetItemTextLong(int nRow, int nCol) const;
    virtual ~CGridCtrl();
