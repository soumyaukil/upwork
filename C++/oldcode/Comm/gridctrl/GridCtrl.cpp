...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_VScrollPos=0;
	m_HScrollPos=0; 
	m_HScrollBar=NULL;	//modified
	m_VScrollBar=NULL;	//modified
...
BOOL CGridCtrl::CreateFromStatic(UINT nID, CWnd* parent)
{
	CWnd *pwnd;
	pwnd=parent->GetDlgItem(nID);
	if (pwnd==NULL) return FALSE;
	CRect rc;
	pwnd->GetWindowRect(&rc);
	parent->ScreenToClient(&rc);
	pwnd->DestroyWindow();
	return Create(rc,parent,nID);
}
...
//        nHorzScroll = GetScrollPos(SB_HORZ);
	int nVertScroll, nHorzScroll;
	if (m_VScrollBar) nVertScroll=m_VScrollBar->GetScrollPos();
	else              nVertScroll=GetScrollPos(SB_VERT);
	if (m_HScrollBar) nHorzScroll=m_HScrollBar->GetScrollPos();
	else			  nHorzScroll=GetScrollPos(SB_HORZ);
...
int CGridCtrl::GetScrollPos32(int nBar, BOOL bGetTrackPos /* = FALSE */)
{
	...
	CScrollBar * pscrollbar=NULL;	//add by tom
	if (nBar==SB_HORZ ) 
		pscrollbar=m_HScrollBar; //add by tom
	else if(nBar==SB_VERT) 
		pscrollbar=m_VScrollBar;//add by tom
	else pscrollbar=NULL;

    if (bGetTrackPos)
    {	// si.fMask=SIF_TRACKPOS;
		if (pscrollbar==NULL){							//add by tom
    ...
    else
    {
		...
		if (pscrollbar==NULL){							//add by tom
			...
			if (pscrollbar->GetScrollInfo(&si,SIF_POS))
				return si.nPos;
		}
    }
...
BOOL CGridCtrl::SetScrollPos32(int nBar, int nPos, BOOL bRedraw /* = TRUE */)
{
	...
	if (pscrollbar!=NULL) {
		return pscrollbar->SetScrollInfo( &si, bRedraw);
	}else {
...
void CGridCtrl::EnableScrollBars(int nBar, BOOL bEnable /*=TRUE*/)
{
    if (bEnable)
    {
        if (!IsVisibleHScroll() && (nBar == SB_BOTH || nBar == SB_HORZ))
        {
			...
			if (m_HScrollBar==NULL) {	// add by tom
				CWnd::EnableScrollBarCtrl(SB_HORZ, bEnable);
			}else {
				m_HScrollBar->EnableWindow(bEnable);
			}
        }
        ...
        if (!IsVisibleVScroll() && (nBar == SB_BOTH || nBar == SB_VERT))
        {
			...
			if(m_VScrollBar==NULL) {	//add by tom
			   CWnd::EnableScrollBarCtrl(SB_VERT, bEnable);
			} else {	
				m_VScrollBar->EnableWindow(bEnable);
			}
        }
    }
    else
...
void CGridCtrl::ResetScrollBars()
{
    ...
    if (IsVisibleVScroll()&&(m_VScrollBar==NULL))
        rect.right += GetSystemMetrics(SM_CXVSCROLL) + GetSystemMetrics(SM_CXBORDER);
    
    if (IsVisibleHScroll()&&(m_HScrollBar==NULL))
    ...
    if ((VisibleRect.Height() < VirtualRect.Height())&&(m_HScrollBar==NULL)) 
        VisibleRect.right -= ::GetSystemMetrics(SM_CXVSCROLL);
    // If horz scroll bar, vert space is reduced
    if ((VisibleRect.Width() < VirtualRect.Width())&&(m_VScrollBar==NULL))
	...
	if (m_HScrollBar!=NULL) {
		...
		if (m_nHScrollMax==0) {
			m_HScrollBar->EnableWindow(FALSE);
		}
	}else {
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
    ...
    for (col = nFirstColumn; col < GetColumnCount(); col++)
    {
		...
		if (m_arColWidths[col] <=0)	m_arColWidths[col]=1;// don't hide the column! ---TOM LIU
    }
...
//modified
long CGridCtrl::GetItemTextLong(int nRow, int nCol) const
{
    if (nRow < 0 || nRow >= m_nRows || nCol < 0 || nCol >= m_nCols)
        return 0;
    ...
    if (!pCell)
        return 0;
    return atoi(pCell->GetText());
}

BOOL CGridCtrl::SetItemTextLong(int nRow, int nCol, long val)
{
	char str[40];
	sprintf(str,"%d",val);
    CGridCellBase* pCell = GetCell(nRow, nCol);
...
int CGridCtrl::SetScrollBar(CScrollBar *phscrollbar, CScrollBar *pvscrollbar)
{
	EnableScrollBar(SB_BOTH,FALSE);
	m_HScrollBar=phscrollbar;
	m_VScrollBar=pvscrollbar;
//	EnableScrollBar(SB_BOTH,FALSE);
	if (IsWindow (m_hWnd)) {
		if (phscrollbar) CWnd::EnableScrollBarCtrl(SB_HORZ, FALSE);
		if (pvscrollbar) CWnd::EnableScrollBarCtrl(SB_VERT, FALSE);
		ResetScrollBars();
	}
	return 0;
...
BOOL CGridCtrl::SortTextItems(int nCol, BOOL bAscending, int low, int high)
{
    ...
    ResetSelectedRange();
    SetFocusCell(-1, - 1);
    return CGridCtrl::SortItems(pfnCellTextCompare, nCol, bAscending, 0,low,high);
}

BOOL CGridCtrl::SortNumItems(int nCol, BOOL bAscending)
{
    m_nSortColumn = nCol;
    m_bAscending = bAscending;
    ResetSelectedRange();
    SetFocusCell(-1, - 1);
    return SortNumItems(nCol, bAscending, GetFixedRowCount(), - 1);
}

BOOL CGridCtrl::SortNumItems(int nCol, BOOL bAscending, int low, int high)
{
    if (nCol >= GetColumnCount())
        return FALSE;
    ...
    double midItem = atof(GetItemText((lo + hi)/2, nCol));
    ...
    // must now sort the left partition.
    if (low < hi)
        SortNumItems(nCol, bAscending, low, hi);
    
    // If the left index has not reached the right side of array
    // must now sort the right partition.
    if (lo < high)
        SortNumItems(nCol, bAscending, lo, high);
...
void CGridCtrl::EnableDragAndDrop(BOOL bAllow )    {
	if(bAllow==m_bAllowDragAndDrop) return;
